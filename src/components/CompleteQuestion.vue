    ...
    <div class="ucb-question complete-question">
      ...
      <span v-for='(ask, index) in repeatDropZone'>
        ...
        <div class="drag" :id="'list_' + index" v-bind:class="borderFeedback[index]" v-if="repeatDropZone.length-1 > index">
            <draggable v-model="list['list_' + index]" class="dragArea" :options="{group:'answers'}" @end="verifyIfAllAnswerChecked" :move="checkMoveAsk">
            <div class="drop" :data-position="element.position" v-for="element in list['list_' + index]">{{element.word}}</div>
            </draggable>
        </div>
      </span>

      <div class="answer-box">
        <p>{{ $t('questionDragDropInstruction') }}</p>
        <draggable v-model="question.answers" class="dragArea answer-options" @start="drag=true" :options="{group:'answers'}" :move="checkMove" @end="verifyIfAllAnswerChecked">
          <div v-for="element in question.answers" :key="element.position">{{element.word}}</div>
        </draggable>
      </div>
...
export default {
  ...
  computed: {
    repeatDropZone: function () {
      var arrayQuest = this.question.ask.split(/{{.*?}}/g)
      return arrayQuest
    },
  ...
  methods: {
    checkMove: function (evt) {
      if (evt.relatedContext.list && evt.relatedContext.list.length > 0) {
        return false
      }
    },
    checkMoveAsk: function (evt) {
      if (!evt.to.classList.contains('answer-options')) {
    ...
    verifyAnswer: function () {
      this.feedback = true
      Object.entries(this.list).forEach(([index, item]) => {
        if (item[0]) {
          var positionList = 'list_' + item[0].position
          var positionQuestion = parseInt(index.split('list_')[1])
          if (positionList !== index || item[0].value !== true) {
            ...
            document.getElementById('list_' + positionQuestion).classList.add('border__valid')
          }
        }
      })
    ...
    verifyIfAllAnswerChecked: function () {
      var countAnsw = this.question.countTrueAnswers.filter(function (e) { return e.value }).length
      var countResp = 0
      Object.entries(this.list).forEach(([index, item]) => {
        if (item[0]) {
          countResp++
      ...
      if (countAnsw === countResp) {
        this.allAnswerChecked = true
      } else {
        this.allAnswerChecked = false
      }
    },
  ...
  created: function () {
    ...
    for (let i = 0; i < this.repeatDropZone.length - 1; i++) {
      this.list['list_' + i] = []
    }
