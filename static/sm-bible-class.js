...
var smBibleClassModal = new Modal({})

var triggerButton = document.querySelectorAll('.saiba-mais-widget')
var widgetURL = 'https://static.saibamais.org.br'

var openModalListener = function () {
  var settings = {}
  for (var i = 0; i < this.attributes.length; i++) {
    var attr = this.attributes[i]
    if (attr.name.substring(0, 5) == 'data-') {
      settings[attr.name.substring(5)] = attr.value
    }
  }
  settings = Object.assign(settings, window.smBibleClassSettings)
  var lessonURL = `${widgetURL}/?${serialize(settings)}`
  smBibleClassModal.open(lessonURL)
}

for (var i = 0; i < triggerButton.length; i++) {
  triggerButton[i].addEventListener('click', openModalListener)
}
